...
if (empty($_POST['page'])) {  // When no page is sent from the client; The initial display
    ...
    include 'w6_view_startpage.php';  // Start page with no modal window
    exit();
}
...
if ($_POST['page'] == 'StartPage')
{
    ...
    switch($command) {  // When a command is sent from the client
        case 'SignIn':  // With username and password
            ...
            } else  // If the user is valid
                include 'w6_view_mainpage.php';
            exit();
        ...
        case 'Join':  // With username, password, email, some other information
			...
			$password = $_POST['password'];
			$email = $_POST['email'];
            if (does_exist($username)) {  // If the user exists, redisplay 'Join' modal window
                $error_msg_username = '* The user exists.';
                $display_type = 'join';
                include 'w6_view_startpage.php';
            } else {  // If the user does not exist
                if (insert_new_user($username, $password, $email)) {  // Insert the new user. If there is no error, display 'SignIn' modal window.
                    $display_type = 'signin';
                    include 'w6_view_startpage.php';
